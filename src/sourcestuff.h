...
#ifdef GMOD 
	RENDERVIEW_GMOD_NO_POSTPROCESS = ( 1 << 3 ),
	RENDERVIEW_GMOD_ONLY_POSTPROCESS = ( 1 << 4 )
#endif
};
...
public:
	...
	virtual void GMOD_SetTimeManipulator( float f ) = 0;
	virtual void GMOD_SendToServer( void* pData, unsigned int iSize, bool bReliable = true ) = 0;
	virtual void GMOD_PlaceDecalMaterial( IMaterial *pMaterial, bool bBrush, int index, IClientEntity* entity, const Vector &position, const Vector &normal, const color32 &rgbaColor, float x, float y ) = 0;
	virtual void GMOD_GetSpew( char *buf, size_t buflen ) = 0;
	virtual void GMOD_SetViewEntity( unsigned int i ) = 0;
	virtual void GMOD_BrushMaterialOverride( IMaterial *pMaterial ) = 0;
	virtual void GMOD_R_RedownloadAllLightmaps() = 0;
...
public:
	...
	// GMOD 
	virtual void			GMOD_ReceiveServerMessage( bf_read *buf, int length ) = 0;
	virtual void			GMOD_DoSnapshots() = 0;
	virtual void			GMOD_VoiceVolume( unsigned int iEntity, float fVolume ) = 0;
	virtual void			GMOD_OnDrawSkybox() = 0;
	virtual void			IN_MouseWheelAnalog( int iDelta ) = 0;
	// GMOD 
};
